...
%\fi
\\\solution \\
Define a random variables X as:
\begin{table}[H]
        \centering
        \input{tables/main.tex}
        \caption{Random Variables}
        \label{tab:11.16.3.33}
\end{table}
\noindent Given that,
\begin{align}
\pr{(X=0)(X=1)} &\leq 0.3\\
...
\begin{align}
\pr{(X=0) + (X=1)} &\leq 1\\
\pr{(X=0) + (X=1)} &= p_X(0) + p_X(1) - \pr{(X=0)(X=1)}\\
\implies \pr{(X=0)(X=1)} &= p_X(0) + p_X(1) - \pr{(X=0) + (X=1)}
\end{align}
\begin{align}
\implies p_X(0) + p_X(1) - \pr{(X=0) + (X=1)} &\leq 0.3\\
\implies p_X(0) + p_X(1) &\leq 0.3 + \pr{(X=0) + (X=1)}\\
\implies p_X(0) + p_X(1) &\leq 0.3 + 1\\
\implies 0.5 + p_X(1) &\leq 1.3\\
\implies p_X(1) &\leq 0.8
\end{align}
